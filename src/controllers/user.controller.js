...
const generateAccessAndRefreshTokens = async(userId) => {
  try {
      ...
      throw new ApiError(500,"Something went wrong when generating refersh and access tokens")
  }
}
...
const refershAcessToken = asyncHandler(async (req,res) => {
  ...
  try {
    ...
    const {accessToken,newRfreshToken} = await generateAccessAndRefreshTokens(user._id)
    
    return res
    .status(200)
    .cookie("accessToken",accessToken,options)
    .cookie("refreshToken",newRfreshToken,options)
    .json(new ApiResponse(200,{accessToken,refreshToken:newRfreshToken},"Access token  Refresh  sucessfully !!!"))
  } catch (error) {
...
const upadateAccoountDetails = asyncHandler(async (req,res) => {
      ...
      return res
      .status(200)
      .json(new ApiResponse(200,user,"Accountdetails  upadated successfully !!"))
})
